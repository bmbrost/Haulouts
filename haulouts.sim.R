...
eta <- c(rbeta(H-1,1,a0),1)  # stick-breaking weights
...
priors <- list(H=H,r=4,q=2,sigma.l=0,sigma.u=5,sigma.mu.l=0,sigma.mu.u=5,sigma.beta=1)
...
out1 <- haulouts.1.mcmc(s,y,X[s.idx,],X[-s.idx,],W[s.idx,],W[-s.idx,],
	S,S.tilde,sigma.alpha=2,priors=priors,tune=tune,start=start,n.mcmc=20000)
