...
priors <- list(H=H,r=4,q=2,sigma.l=0,sigma.u=5,sigma.mu.l=0,sigma.mu.u=5,
	alpha=1,beta=1,sigma.beta=1,mu.gamma=rep(0,qU),sigma.gamma=1)
tune <- list(sigma=0.05,sigma.mu=0.25,mu.0=1)
# hist(rgamma(1000,4,2))
# hist(rgamma(1000,5,2.5))
out1 <- haulout.dpmixture.3.mcmc(s,y,X[s.idx,],X[-s.idx,],W[s.idx,],W[-s.idx,],
	S,S.tilde,U,sigma.alpha=2,priors=priors,tune=tune,start=start,n.mcmc=2000)
...
points(h,pch=1,cex=1,col=rgb(1,0,0,1))
points(s,pch=19,cex=0.2,col=3)
points(S.tilde+cbind(rep(-1.5,n.haulout),0),cex=selection/max(selection))
...
matplot(mod$gamma[idx,],type="l"); abline(h=gamma,col=1:3,lty=2) 
...
v <- apply(out1$beta[idx,],1,function(x) X[s.idx,]%*%x)+
...
# Inference for y
v.tilde.inv <- matrix(pnorm(out1$v[,idx]),,T,byrow=TRUE)
