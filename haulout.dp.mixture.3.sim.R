...
U[,-1] <- scale(U[,-1])
gamma <- c(2,8)  # Haul-out site selection coefficients
...
tune <- list(sigma=0.05,sigma.mu=0.2,mu.0=2,gamma=3)
...
abline(v=S.tilde[1,1],col=3,lty=2)
...
# Inference for y
v.tilde.inv <- matrix(pnorm(mod$v[,idx]),,T,byrow=TRUE)
