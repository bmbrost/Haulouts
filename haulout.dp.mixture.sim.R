...
S.tilde <- cbind(c(max(S.bar[,1]),2,2,max(S.bar[,1]),max(S.bar[,1])),S.bar[,2])  # support 
	# of haul out process
...
theta <- cbind(runif(H,min(S.tilde[,1]),max(S.tilde[,1])),
	runif(H,min(S.tilde[,2]),max(S.tilde[,2])))  # clusters randomly drawn from P0
...
out1 <- dpmixture.blocked.2d.mcmc(y,P0,
  ...
  tune=list(z=0.5,sigma=0.01),start=start,n.mcmc=10000)
