...
L <- c(-10,-2) # Land domain
S.tilde <- c(max(L),2) # Support of haul out process
S <- c(max(L),10) # Support of movement process (marine and haul-out environments)
...
out1 <- haulout.mcmc(s[,1],mu[,1],S.tilde,S,priors=list(alpha=1,beta=1,q=3,r=2),
