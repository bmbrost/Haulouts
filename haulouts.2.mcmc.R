haulouts.2.mcmc <- function(s,y,X,X.tilde,W,W.tilde,S.tilde,sigma.alpha,
	...
	# library(msm)  # for truncated normal density
  	...
  	# get.mh.mu.0 <- function(x,mu.0,mu.0.star,S.tilde,ht,s,lc,z,
		# Sigma.inv,Sigma.det,Sigma.z0.inv,Sigma.z0.det){
		# browser()
		...
		# mh.star <- sum(
			# ldmvnorm2(s[idx.1,],mu.0.tmp.star,lc,Sigma.inv,Sigma.det,const=TRUE),
			# ldmvnorm2(s[idx.0,],mu.0.tmp.star,lc,Sigma.z0.inv,Sigma.z0.det,const=TRUE)) 
			# # sum(dnorm(s[idx.1,1],mu.0.tmp.star[1],sigma,log=TRUE)+
		...
		# mh.0 <-	sum(
			# ldmvnorm2(s[idx.1,],mu.0.tmp,lc,Sigma.inv,Sigma.det,const=TRUE),
			# ldmvnorm2(s[idx.0,],mu.0.tmp,lc,Sigma.z0.inv,Sigma.z0.det,const=TRUE)) 
			# # sum(dnorm(s[idx.1,1],mu.0.tmp[1],sigma,log=TRUE)+
	...
	dmvnorm2 <- function(x,y,lc,Sigma,const=TRUE,log=TRUE){
	...
	get.ht <- function(x,y,z,lc,Sigma,Sigma.z0,const=FALSE){
	 	#Calculate log density of mixture normal distribution (Eq. 1)
		# browser()	
		K <- matrix(c(-1,0,0,1),2)  # mixture transformation matrix
...
# browser()
		...
		linpred <- X.comb%*%beta+W.comb%*%alpha
		p <- pnorm(linpred[1:T,])
		p.tmp1 <- p*dmvnorm2(s,S.tilde[ht,3:4],lc,Sigma,const=TRUE,log=FALSE)
		p.tmp2 <- (1-p)*dmvnorm2(s,S.tilde[ht,3:4],lc,Sigma.z0,const=TRUE,log=FALSE)
